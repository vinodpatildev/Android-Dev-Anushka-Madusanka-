package com.vinodpatildev.customfancontroller
...
class DialView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : View(context,attrs,defStyleAttr) {
    ...
    override fun onSizeChanged(width: Int, height: Int, oldwidth: Int, oldheight: Int) {
        Log.d("mytag","onSizeChanged($width,$height,$oldwidth,$oldheight)")
        super.onSizeChanged(width, height, oldwidth, oldheight)
        radius = (min(width, height) / 2.0 * 0.8).toFloat()
    }
    private fun PointF.computeXYForSpeed(pos: FanSpeed, radius: Float) {
        Log.d("mytag","computeXYForSpeed($pos,$radius)")
